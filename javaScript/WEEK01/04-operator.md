...
- `null` 값과 배열을 넣어보면 "object"`로 반환됨.

- 선언하지 않은 식별자를 연산하는 경우, ReferenceError가 발생하지 않고 `"undefined"`가 반환됨.
...
- `**`
  ...
  - 좌항의 피연산자를 밑으로, 우항의 피연산자를 지수로 <b>거듭 제곱</b>한 숫자 값을 반환함.
...
- 음수를 밑(좌항의 피연산자)으로 사용하려면 괄호로 묶어야 됨.

  - 밑이 음수인데 지수가 정수가 아닌 경우 `NaN`이 반환됨.

- 지수 연산자도 다른 산술 연산자와 마찬가지로, 할당 연산자(`=`)와 함께 사용할 수 있음.

- 지수 연산자는 이항 연산자 중에서 우선순위가 가장 높음.

- 지수 연산자 도입 전에는 `Math.pow` 메서드를 사용했음.
...
- 어떤 연산자가 다른 코드에 영향을 주는 것을 부수 효과라고 하며, 부수 효과는 일부 연산자만이 가지는 특징임.

- 부수 효과를 갖는 연산자

  1. 증가/감소 연산자(`++`/`--`)

    - 부수 효과 : 피연산자의 값이 재할당되어 변경됨.

  2. 할당 연산자(`=`)

    - 부수 효과 : 좌항의 피연산자, 즉 변수의 값이 변함.
...
- 연산자 우선순위란, 여러 개의 연산자로 이루어진 문이 실행될 때 <b>연산자가 실행되는 순서</b>를 의미함.

- 연산자 우선순위를 외워서 쓰는 건 어려우니, 그룹 연산자`()`를 사용하여 우선순위를 명시적으로 조절하면 됨.

___
## 연산자 결합 순서

- 연산자 결합 순서란, 연산자의 어느 쪽부터 평가를 수행할 것인지를 의미함.
